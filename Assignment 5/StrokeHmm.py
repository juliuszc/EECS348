...
class HMM:
    ...
    def label( self, data ):
        ...
        for state in self.states:
            # for priors/emissions
            V[0][state]=self.priors[state]*listProduct(self.emissions[state][feature][data[0][feature]] for feature in self.featureNames)
            path[state]=[state]
        for t in range(1,len(data)):
            V.append({})
            newPath={}
            for state in self.states:  #for each possible state, return most Likely State
                probability,mostLikelyState=max((V[t-1][s]*self.transitions[s][state]*listProduct(self.emissions[state][feature][data[t][feature]] for feature in self.featureNames),s) for s in self.states)
...
class StrokeLabeler:
    def __init__(self):
        ...
        self.featureNames = ['length', 'curvature', 'boxArea']
        self.contOrDisc = {'length': DISCRETE, 'curvature':DISCRETE, 'boxArea':DISCRETE}
        self.numFVals = { 'length': 2, 'curvature': 2, 'boxArea': 2}
    ...
    def featurefy( self, strokes ):
        ...
        for s in strokes:
            ...
            curve = s.sumOfCurvature(lambda x:math.fabs(x), 9)
            if curve < curveLimit:
                d['curvature'] = 0
            else:
                d['curvature'] = 1

            boxArea = s.boxAreaCalculator()
            if boxArea < 100000:
                d['boxArea'] = 0
            else:
                d['boxArea'] = 1
...
class Stroke:
    ...
    def boxAreaCalculator(self):
        #calculates the area of a box required to surround the entire drawing/text
        left = float("inf")
        right = 0
        up = 0
        down = float("inf")
        for p in self.points: #test bounds
            if p[0] < left:
                left = p[0]
            if p[0] > right:
                right = p[0]
        ...
        length = right - left
        height = up - down
        area = length * height #def of area of a rectangle
        return area
